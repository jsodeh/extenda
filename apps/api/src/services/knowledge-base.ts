...
import { embedContent, ModelConfig } from '../lib/models.js';
...
export class KnowledgeBaseService {
     ...
     */
    async ingest(userId: string, filename: string, content: string, metadata: any = {}, modelConfig?: ModelConfig) {
        ...
        for (const chunk of chunks) {
            try {
                ...
                const embedding = await embedContent(chunk, modelConfig);
     ...
     */
    async search(userId: string, query: string, limit: number = 3, modelConfig?: ModelConfig): Promise<string[]> {
        console.log(`Searching knowledge base for: "${query}"`);

        try {
            // 1. Embed query
            const queryEmbedding = await embedContent(query, modelConfig);
